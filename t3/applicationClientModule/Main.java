...
//						}
	...
	public static void main(String[] args) throws InvalidFormatException, IOException{
		String path="D:/Downloads/BPM2017_hw1(withdata）/ProMdata/12.xlsx";
		File file=new File(path);
		File file3=new File("E:/bank/guocheng.txt");
		FileWriter fileWriter = new FileWriter(file3);
		XSSFWorkbook wb= new XSSFWorkbook(file);
		XSSFSheet sheet=wb.getSheetAt(0);
		String id="0000";
		for(int i=1;sheet.getRow(i)!=null;i++){
			...
			if(!temp.equals(id)){
				fileWriter.write("</trace>\r\n<trace>\r\n<string key=\"concept:name\" value=\""+temp+"\" />\r\n");
				id=temp;
			}
			Date td=row.getCell(3).getDateCellValue();
			DateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSXXX");			
			fileWriter.write("<event>\n");
			fileWriter.write("<string key=\"org:resource\" value=\""+row.getCell(1).getStringCellValue()+"\" />\r\n");
			fileWriter.write("<date key=\"time:timestamp\" value=\""+df.format(td)+"\" />\r\n");
			fileWriter.write("<string key=\"concept:name\" value=\""+row.getCell(5).getStringCellValue()+"\" />\r\n");
			fileWriter.write("<string key=\"lifecycle:transition\" value=\"complete\"/>\r\n");
			fileWriter.write("</event>\r\n");
		}
		fileWriter.write("</trace>\r\n");
...
/*		
		...
		for(int i=1;sheet.getRow(i)!=null;i++){
			...
			if(row.getCell(1)!=null){
				row.getCell(1).setCellType(CellType.STRING);
				map.put(row.getCell(1).getStringCellValue(), 1);
				map3.put(row.getCell(1).getStringCellValue(), 1);
				row.getCell(0).setCellType(CellType.STRING);
				map.put(row.getCell(0).getStringCellValue(), 1);
				map3.put(row.getCell(0).getStringCellValue(), 1);
			}
		}
		path="E:/bank/key_value_temp.xlsx";
		file=new File(path);
		...
		for(int i=1;sheet.getRow(i)!=null;i++){
			...
			if(row.getCell(0)!=null){
				row.getCell(1).setCellType(CellType.STRING);
				map2.put(row.getCell(1).getStringCellValue(), 1);
				map3.put(row.getCell(1).getStringCellValue(), 1);
				row.getCell(0).setCellType(CellType.STRING);
				map2.put(row.getCell(0).getStringCellValue(), 1);
				map3.put(row.getCell(0).getStringCellValue(), 1);
			}
		}
		path="E:/bank/1.xlsx";
		...
		for(int i=1;sheet.getRow(i)!=null;i++){
			...
			if(row.getCell(0)!=null){
				row.getCell(0).setCellType(CellType.STRING);
				map4.put(row.getCell(0).getStringCellValue(), 1);
				map3.put(row.getCell(0).getStringCellValue(), 1);
			}
		}
		...
		for(String s:map3.keySet()){
			if(map3.get(s)==0)	continue;
			for(String s2:map3.keySet()){
				if(map3.get(s2)==0)	continue;
				if(!s.equals(s2)&&s.contains(s2)){
					count++;
					map3.put(s2, 0);
				}
			}
